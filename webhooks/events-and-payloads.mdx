...
For the HTTP contract, `data` key mapping, full allow-list of event types, and integration checklist, see the [Webhook developer guide](/webhooks/developer-guide).
...
- [participant.assigned.round](#participantassignedround) — Participant assigned to a round
- [participant.assigned.group](#participantassignedgroup) — Participant assigned to a group
- [participant.assigned.match](#participantassignedmatch) — Participant assigned to a match
- [participant.removed.round](#participantremovedround) — Participant removed from a round
- [participant.removed.group](#participantremovedgroup) — Participant removed from a group
- [participant.removed.match](#participantremovedmatch) — Participant removed from a match
...
**Tournaments**
- [tournament.published](#tournamentpublished) — Tournament published
- [tournament.rules.updated](#tournamentrulesupdated) — Tournament rules updated
- [tournament.points.updated](#tournamentpointsupdated) — Tournament points system updated

**Match announcements**
- [id.pass.announcement](#idpassannouncement) — Room ID and credentials for a match
...
**When:** Participants are assigned to a round (for example bracket slots).
...
**When:** Participants are assigned to a group (for example within a round).
...
| `participantIds` | array  | UUIDs of assigned participants (may be omitted until the match tracks them) |
| `participantType`| string | `"User"` or `"Team"`                 |
...
**Payload `data`:** Same fields as `participant.assigned.round` (`tournamentId`, `roundId`, `roundName`, `participantIds`, `participantType`).
...
**Payload `data`:** Same fields as `participant.assigned.group`.
...
**Payload `data`:** Same fields as `participant.assigned.match`.
...
| `tournamentId`| string | UUID of the tournament     |
| `matchId`     | string | UUID of the match          |
| `messageContent`| string | Room ID and credentials   |
