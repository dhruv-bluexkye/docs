...
- [participant.assigned](#participantassigned) — Participant assigned to a match, round, or group
- [participant.removed](#participantremoved) — Participant removed from a round/group
...
**When:** Participants are assigned to a specific round, group, or match.
...
<Info>
The shape of `participant.assigned` changes depending on the context of the assignment. If assigned directly to a match, the `matchId` is included but `participantIds` might be omitted from the payload until the match specifically tracks them.
</Info>
...
**When:** A participant is removed from a round or group.
